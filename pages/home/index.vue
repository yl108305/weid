<template>
	<view>
		...
		<view class="flexView">
			<view class="scrollView">
				<view class="head-read">
					<view class="flex">
						<image class="read-img" src="../../static/WE.jpg" mode="aspectFill" />
						<view class="flex-box">
							<view class="flex-box-text">姓名：{{ userData.name }}({{ userData.id }})</view>
							<view class="flex-box-text">手机号：{{ userData.tel }}</view>
						</view>
						<view class="arrow arrow-one" @click="navTo('/pages/password/password')"><span>修改密码</span></view>
					...
					<view class="tx">
						<view class="tx-grid">
							<view class="tx-grid-text">
								...
								<view class="money">
									<text>{{ userData.rmb }}</text>
									<text class="money-b">元</text>
								</view>
							</view>
						</view>
						<view class="tx-grid"   @click="navTo('/pages/equipment/equipment')"><span class="tx-grid-comm-sign" style="border-radius:40rpx 40rpx 40rpx 40rpx;">设备管理{{countdb}}</span></view>
				...
				<view class="list-view">
					<view class="list-item" v-for="(item, index) in list" :key="index" hover-class="hover" :hover-stay-time="150" bindtap="detail">
						<view class="content-box">
							<view class="des-box">
								<view class="tit">{{ currentTab == 3 ? '流水号：' + item.order : '订单号：' + item.order }}</view>
								<view v-if="currentTab == 3" class="source" :style="{ color: item.statue == 2 ? '#4caf50' : item.statue == 1 ? '#ff1e0f' : '#00b7ff' }">
									提现{{ item.statue == 2 ? '成功' : item.statue == 1 ? '失败' : '处理中' }}
								</view>
								<view v-else style="margin-top: 10rpx;margin-bottom: 10rpx;">{{item.address}}</view>
								<view class="time">{{ item.date }}</view>
							</view>
						</view>
						<view class="money" :class="{ less: is_withdraw }">{{ is_withdraw ? '-' : '+' }}{{ currentTab == 3 ? item.rmb : item.rmb }}</view>
					</view>
				</view>
		...
		<view class="go_auth" @click="go_auth">
			免费用户
		</view>
	</view>
</template>
